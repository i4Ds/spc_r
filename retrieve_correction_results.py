...
def main():
    parser = argparse.ArgumentParser(
        description="Retrieve CORRECTION batch job results from OpenAI's Batch API."
    )
    parser.add_argument(
        "--batch_job_id",
    ...
    if args.batch_job_id_folder:
        batch_id_files = glob(
            os.path.join(args.batch_job_id_folder, "*.jsonl.batch_id")
        )
        # Filters, which are coming from judgement, filter them out.
        batch_id_files = [
            f for f in batch_id_files if "corrected" not in os.path.basename(f)
        ]
